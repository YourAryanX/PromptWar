...
export default function IdeasPage() {
  ...
  const generateIdeas = async () => {
    ...
    setIdeas([])
    try {
      // Skills are passed from the server — in production this would come from the profile
      const res = await fetch('/api/generate-ideas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          skills: [],  // populated from profile via API in next iteration
          interests: 'Web apps with AI features'
        }),
      })
  ...
  const handleConfirmProject = async () => {
    ...
    // createProject redirects to /dashboard on success
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-12 relative overflow-hidden flex flex-col items-center">
      ...
      <motion.div 
        ...
        className="fixed top-0 right-0 w-[40rem] h-[40rem] bg-indigo-500/20 rounded-full blur-[120px] -z-10 pointer-events-none"
      />
      <motion.div 
        animate={{ scale: [1, 1.1, 1], opacity: [0.2, 0.4, 0.2] }}
        transition={{ duration: 20, repeat: Infinity, ease: "linear", delay: 1 }}
        className="fixed bottom-0 left-0 w-[35rem] h-[35rem] bg-rose-500/20 rounded-full blur-[120px] -z-10 pointer-events-none"
      />
